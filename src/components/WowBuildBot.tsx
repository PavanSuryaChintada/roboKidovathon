...
export const WowBuildBot: React.FC = () => {
  const annotations = [
    ...
    { label: 'IR SENSOR ARRAY', spec: '8-channel · Sub-ms', pos: 'top-[32%] left-[3%]', lineDir: 'right' },
    { label: 'MCU CORE', spec: 'MicroPython / C++', pos: 'top-[56%] left-[4%]', lineDir: 'right' },
  ...
  return (
    <section className="w-full bg-[#07111F] text-white overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-6 sm:px-10 py-20 md:py-28">
        {/* Section heading */}
        <div className="mb-14">
          <span className="text-[10px] font-mono font-bold tracking-[0.25em] text-white/35 uppercase block mb-4">
        ...
        <div className="relative w-full overflow-hidden" style={{ aspectRatio: '16/7' }}>
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-[#07111F]/65 via-transparent to-[#07111F]/65 pointer-events-none" />
          <div className="absolute inset-0 bg-gradient-to-b from-[#07111F]/30 via-transparent to-[#07111F]/55 pointer-events-none" />

          {/* Annotation labels — blue labels, yellow dots */}
          {annotations.map((ann, i) => (
            <div key={i} className={`absolute ${ann.pos} flex items-center gap-2 ${ann.lineDir === 'right' ? 'flex-row' : 'flex-row-reverse'}`}>
              <div className={ann.lineDir === 'right' ? 'text-left' : 'text-right'}>
                <span className="block text-[9px] font-mono font-bold tracking-[0.2em] text-[#0052FF] uppercase">
                  {ann.label}
                </span>
        ...
        {/* Bottom spec row */}
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 divide-x divide-white/8 border-t border-white/8 pt-8">
          {[
            ...
            <div key={i} className="px-5 first:pl-0 last:pr-0 py-2">
              <span className="block text-[9px] font-mono text-[#0052FF] mb-1">{item.num}</span>
              <span className="block text-xs font-bold text-white uppercase tracking-wider mb-1">{item.title}</span>
              <span className="block text-[11px] text-white/40 font-light leading-relaxed">{item.spec}</span>
            </div>
          ))}
        </div>
      </div>
