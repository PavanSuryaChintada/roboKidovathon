    ...
    <section className="w-full bg-[#080D1D] text-white py-16 md:py-20 border-b border-slate-800">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-10 gap-4">
          ...
          <p className="text-sm sm:text-base font-inter text-[#E2E8F0] font-medium max-w-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {blocks.map((item, idx) => (
            <div
              key={idx}
              className="group card-dark overflow-hidden flex flex-col justify-between border border-slate-800"
            >
              <div className="relative h-56 sm:h-64 w-full overflow-hidden bg-[#050814]">
                <img
                  src={item.image}
                  alt={item.title}
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-90"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#141C2E] via-transparent to-transparent" />
                <span className="absolute bottom-3 left-3 text-[11px] font-mono font-bold text-white tracking-wider bg-[#080D1D]/90 px-2.5 py-1 rounded border border-slate-700">
                  {item.tag}
                </span>
              </div>

              <div className="p-6 space-y-1.5 bg-[#141C2E] flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-white text-balance">
                    {item.title}
                  </h3>
                  <p className="text-xs text-[#E2E8F0] font-inter mt-1 leading-relaxed font-medium">
                    {item.desc}
                  </p>
                </div>
              </div>
