  ...
  return (
    ...
    >
      <div className="max-w-[1440px] mx-auto px-6 sm:px-10 flex items-center justify-between gap-6">
        ...
        {/* Brand Mark (Left - Interactive Color Hover) */}
        <button
          onClick={() => handleLinkClick('home')}
          className="text-left group focus:outline-none flex items-center"
        >
          <span className="font-headline font-bold text-lg sm:text-xl tracking-tight text-white uppercase group-hover:text-[#22C55E] transition-colors duration-200 drop-shadow-sm">
            ROBO-KIDO-A-THON
          </span>
        </button>

        {/* Desktop Navigation Links (Center - Interactive Pill Hover Effects) */}
            ...
            return (
              <button
                ...
                className={`relative px-4 py-1.5 rounded-full text-xs font-display font-medium tracking-wide transition-all duration-200 ${
                  isActive
                    ? 'text-[#22C55E] bg-white/[0.12] font-semibold border border-white/15 shadow-[0_0_15px_rgba(34,197,94,0.2)]'
                    : 'text-slate-300 hover:text-[#22C55E] hover:bg-white/[0.08]'
                }`}
              >
                {item.label}
              </button>
        ...
        {/* Action Button (Right - Scale & Glow on Hover) */}
        <div className="hidden md:flex items-center gap-4">
          <motion.button
            whileHover={{ scale: 1.05, boxShadow: '0 0 20px rgba(34, 197, 94, 0.45)' }}
            whileTap={{ scale: 0.97 }}
        ...
        <button
          ...
          className="md:hidden p-2 text-white hover:text-[#22C55E] focus:outline-none transition-colors"
            ...
            {navLinks.map((item) => (
              <button
                ...
                className={`py-2.5 px-3 rounded-lg text-left text-sm font-display font-medium transition-all ${
                  activeTab === item.id
                    ? 'text-[#22C55E] bg-white/[0.08] font-semibold'
                    : 'text-slate-300 hover:text-[#22C55E] hover:bg-white/[0.04]'
                }`}
              >
                {item.label}
              </button>
