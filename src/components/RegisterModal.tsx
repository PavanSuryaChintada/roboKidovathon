...
export const RegisterModal: React.FC<RegisterModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs overflow-y-auto animate-fadeIn">
      <div className="bg-white border border-slate-200 rounded-lg max-w-xl w-full p-6 sm:p-8 shadow-lg text-slate-900 relative my-8">
        ...
        {statusMessage && (
          <div
            className={`p-4 rounded-md mb-6 flex items-start gap-3 text-xs sm:text-sm border ${
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs sm:text-sm font-semibold">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-md pl-9 pr-4 py-2 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-[#0062B8] transition-colors"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-700 font-bold mb-1">Parent / Teacher Email *</label>
              <div className="relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-700 font-bold mb-1">Age Category *</label>
              <select
                name="ageCategory"
                value={formData.ageCategory}
                onChange={handleChange}
                className="w-full bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-slate-900 focus:outline-none focus:border-[#0062B8] transition-colors"
              >
            ...
            <div>
              <label className="block text-slate-700 font-bold mb-1">Track Interest</label>
              <select
                name="trackInterest"
          ...
          <div className="pt-2 flex items-start gap-2">
            <input
              ...
              onChange={handleChange}
              className="mt-1 rounded bg-white border-slate-300 text-[#0062B8] focus:ring-[#0062B8]"
            />
            <label htmlFor="agreeToRules" className="text-slate-600 text-xs font-normal leading-tight">
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#0062B8] hover:bg-[#00529B] text-white font-bold py-3.5 rounded-md transition shadow-xs flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {loading ? (
                <>
