...
export const EventDeckModal: React.FC<EventDeckModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto select-none">
      <div onClick={onClose} className="fixed inset-0 bg-[#07111F]/85 backdrop-blur-md -z-10" />

      <div className="relative w-full max-w-lg bg-white border border-[#07111F]/15 rounded-[2px] shadow-2xl p-6 sm:p-10 my-8 text-[#07111F]">
        <button
          onClick={onClose}
        ...
        {!downloaded ? (
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#4A5568] font-light mt-2 leading-relaxed">
            ...
            <form onSubmit={handleDownload} className="mt-6 space-y-4">
              ...
              <div className="p-3.5 bg-[#F8F9FA] border border-[#07111F]/10 rounded-[2px] text-[11px] font-mono-code text-[#4A5568] space-y-1">
                <div className="flex items-center gap-1.5 text-[#07111F] font-bold">
          ...
          <div className="py-8 text-center flex flex-col items-center">
            <div className="w-14 h-14 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-600 mb-4">
              <CheckCircle2 className="w-7 h-7" />
            </div>
            <h3 className="font-display font-black text-xl text-[#07111F] uppercase tracking-tight">
              Event Deck Dispatched.
            </h3>
            <p className="text-xs text-[#4A5568] mt-2 max-w-xs leading-relaxed font-light">
              The official prospectus PDF link has been transmitted to <strong className="text-[#07111F]">{email}</strong>.
            </p>
            <button
              onClick={() => {
                setDownloaded(false);
              ...
              className="btn-editorial-dark mt-6 px-6 py-2.5 text-xs"
            >
              CLOSE WINDOW
            </button>
          </div>
